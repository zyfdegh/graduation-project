...
//This method can be used to generate Authorization string, which can be
//set to HTTP Header
//
//Params:
...
func CalcDigestHeader(username string, password string, method string, urlStr string) (digest string, err error) {
	...
	if err != nil {
		logrus.Errorf("fail to send request to gerrit,check gerrit host and port.%v", err)
		return
	}
	...
	if resp.StatusCode == 401 {
		...
		if err != nil {
			//handle exception
			logrus.Errorf("fail to parse url, %v", err)
			return digest, err
		}
